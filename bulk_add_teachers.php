...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['add'])) {
    ...
    for ($count = 0; $count < count($full_name); $count++) {
        ...
        if (isset($full_name[$count], $email[$count], $phne_number[$count], $birth_date[$count], $gender[$count], $address[$count], $qualification[$count])) {
            ...
            {
                $conn->query("INSERT INTO teachers (full_name, email, phone_no, dob, gender, address, qualification) VALUES ('$fullname_clean', '$email_clean', '$phne_number_clean', '$birth_date_clean', '$gender_clean', '$address_clean', '$qualification_clean')");
                $successCount++;
            }
        }
    }
    ...
    if ($successCount > 0) {
        echo "<script>alert('Teachers added successfully');</script>";
        echo "<script>window.location.href = 'http://localhost/sms/manage-teacher.php';</script>";
    } else {
