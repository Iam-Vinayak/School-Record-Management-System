...
if (isset($_POST['input'])) {
    ...
    if (!empty($input)) {
        $query = "SELECT * FROM staff WHERE stf_id LIKE '%$input%' OR full_name LIKE '%$input%' OR qualification LIKE '%$input%' OR email LIKE '%$input%' OR phone_no LIKE '%$input%' OR dob LIKE '%$input%' OR gender LIKE '%$input%' OR address LIKE '%$input%'"; // Use '%' for wildcard search
    } else {
        $query = "SELECT * FROM staff"; // Retrieve all data when input is empty
    }
