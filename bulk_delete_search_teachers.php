...
if (isset($_POST['input'])) {
    ...
    if (!empty($input)) {
        $query = "SELECT * FROM teachers WHERE tr_id LIKE '%$input%' OR full_name LIKE '%$input%' OR qualification LIKE '%$input%' OR email LIKE '%$input%' OR phone_no LIKE '%$input%' OR dob LIKE '%$input%' OR gender LIKE '%$input%' OR address LIKE '%$input%'";
    } else {
    ...
    if (mysqli_num_rows($result) > 0) {
        echo '<table class="bulk-table-details" width="180%">
        <thead>
            <tr>
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            echo "<tr>";
            echo "<td>
            <div class='delete_checkbox'><input type='checkbox' class='checkbox_item' name='delete_multiple_teachers_id[]' value='" . $row['tr_id'] . "'/></div>
            </td>";
